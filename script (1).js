...
function drawLines() {
  ...
  for (const [childId, parents] of Object.entries(connections)) {
    ...
    for (const parentId of parents) {
      ...
      ctx.lineTo(midX, from.y);   // horizontal from parent
      ctx.lineTo(midX, to.y);     // vertical to child's height
      ctx.lineTo(to.x, to.y);     // horizontal to child
      ctx.strokeStyle = isCompleted ? '#00cc55' : 'rgba(255,255,255,0.7)';
      ctx.lineWidth = isCompleted ? 4 : 3;
      ctx.lineJoin = 'round';
      ctx.setLineDash(isCompleted ? [] : []);
      ctx.stroke();
      ctx.setLineDash([]);
    }
  }
